...
3. **Le build écrit `dist/.canopui-version`** — la version réellement embarquée dans le
   bundle. Elle voyage avec `dist/` jusqu'au dossier servi (`C:\QVL\deploy\projectcenter`,
   servi par `serve-vitrine.mjs` derrière cloudflared).
...
**pipeline complète** (lock-guard, build, puis deploy qui rebuild avant de basculer le dossier servi). Si une nouvelle CanopUI casse ce front, la pipeline est
...
2. **Corriger dans CanopUI** et republier (tag `vX.Y.Z`) : `latest` repart en avant, et le
   prochain démarrage — ou un `sudo canopui-autorebuild --force projectcenter` — rattrape le
   front.
